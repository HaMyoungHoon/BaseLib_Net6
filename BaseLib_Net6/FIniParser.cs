...
namespace BaseLib_Net6
{
    ...
    public class FIniParser : IFParser
    {
        ...
        private int GetPrivateProfileString(string section, string key, string def, out string ret, int size, string filePath)
        {
            ret = def;
            ...
            for (int i = posSTX; i < posETX; i++)
            {
                ...
                {
                    ret = buff[i].Substring(buff[i].IndexOf(_equal) + 1);
                    break;
                }
            }
        ...
        private long WritePrivateProfileString(string section, string key, string value, string filePath)
        {
            ...
            ret = 0;
            buff[posKey] = string.Format("{0}{1}", buff[posKey].Substring(0, buff[posKey].IndexOf(_equal) + 1), value);

            return SaveBuff(buff, ret);
        }

        private int PasSTX(List<string> buff, string section)
        {
            int posSTX = -1;
        ...
        private int PasETX(List<string> buff, int posSTX)
        {
            int posETX = -1;
        ...
        private string GetStxSectionEtx(string section) => string.Format("{0}{1}{2}", _stx, section, _etx);
    }
}
